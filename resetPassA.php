...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($new_pass_err) && empty($new_pass_c_err) && empty($email_err)) {
        ...
        $sql = "SELECT staffid, username, password, email FROM staff WHERE email = ?";
        $sql1 = "UPDATE staff SET password = ? WHERE email = ?";
                ...
                if ($stmt->num_rows == 1) {
                    if ($stmt1 = $mysqli->prepare($sql1)) {
                        ...
                        if ($stmt1->execute()) {
                            ...
                            session_destroy();
                            echo "<script>alert('Password Changed Successfully! Redirecting to Login Page..');window.location.href='adminLogin.php';</script>";
                            exit();
                        } else {
        ...
        <div class="center">
            ...
            <form id="resetPassM" name="resetPassM" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" >
                ...
                <div class="signup_link">
                    <a href="adminLogin.php">I've remember my password</a>
                </div>
            </form>
        </div>
    </body>
