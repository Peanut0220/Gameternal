...
$eventname = $eventdate = $eventdesc = $maxperson = "";
$price = 0;
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    } elseif (strlen(trim($_POST["eventdesc"])) > 1000) {
        $eventdesc_err = "Event description Shouldn't exceed 1000 characters";
    } else {
    ...
    if (empty($eventname_err) && empty($eventdate_err) && empty($eventdesc_err) && empty($maxperson_err) && empty($price_err)) {
        $sql = "INSERT INTO event (eventname, eventdate, eventdesc, maxperson, price) VALUES (?, ?, ?, ?, ?);";
        if ($stmt = $mysqli->prepare($sql)) {
            $stmt->bind_param("sssid", $param_eventname, $param_eventdate, $param_eventdesc, $param_maxperson, $param_price);
            $param_eventname = $eventname;
            $param_eventdate = $eventdate;
            $param_eventdesc = $eventdesc;
            $param_maxperson = $maxperson;
            $param_price = $price;
            if ($stmt->execute()) {
                // Redirect to login page
                echo "<script>alert('Event Added Successfully! Redirecting to Admin Event..');window.location.href='adminEvent.php';</script>";
            } else {
